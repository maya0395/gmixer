...
class FashionIQDataset(Dataset):
    ...
    def __init__(self, dataset_path: Union[Path, str], split: Literal['train', 'val', 'test'], dress_types: List[str],
                 mode: Literal['relative', 'classic'], preprocess: callable = None, no_duplicates: Optional[bool] = False,blip_transform: callable = None):
        """
        ...
        print(f"FashionIQ {split} - {dress_types} dataset in {self.mode} mode initialized")
        ...
        try:
            ...
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val']:
                    ...
                    reference_image_path = str(self.dataset_path / 'images_png' / f"{reference_name}.png")
                     
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))

                    target_name = self.triplets[index]['target']
                    target_image_path = str(self.dataset_path / 'images_png' / f"{target_name}.png")
                    ...
                    reference_image_path = str(self.dataset_path / 'images_png' / f"{reference_name}.png")
                    '''
                        For GPT4V api, the preprocess are included in the model. Therefore, only needs the image urls (i.e., image paths)
                    '''
                    return {
                        'reference_image_path': reference_image_path,
                        'reference_name': reference_name,
                ...
                image_path = str(self.dataset_path / 'images_png' / f"{image_name}.png")
                reference_image = self.preprocess(PIL.Image.open(image_path))
                return {
...
class CIRRDataset(Dataset):
   ...
   The dataset can be used in 'relative' or 'classic' mode:
    ...
    def __init__(self, dataset_path: Union[Path, str], split: Literal['train', 'val', 'test'],
        ...
        print( len(self.name_to_relpath) )
    ...
    def __getitem__(self, index) -> dict:
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val']:
                    reference_image_path = str(self.dataset_path / self.name_to_relpath[reference_name])
                    target_hard_name = self.triplets[index]['target_hard']
                    target_image_path = str(self.dataset_path / self.name_to_relpath[target_hard_name])
                    '''
                        For GPT4V api, the preprocess are included in the model. Therefore, only needs the image urls (i.e., image paths)
                    '''
                    return {
                    ...
                    im = PIL.Image.open(reference_image_path)
                    reference_img = self.preprocess(im)
                    return {
...
class CIRCODataset(Dataset):
    ...
    def __init__(self, dataset_path: Union[str, Path], split: Literal['val', 'test'],
                 mode: Literal['relative', 'classic'], preprocess: callable = None, blip_transform: callable = None):
        """
        Args:
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == 'relative':
            ...
            reference_img_path = str(self.img_paths[self.img_ids_indexes_map[reference_img_id]])
            reference_img = self.preprocess(PIL.Image.open(reference_img_path))


            # if self.preprocess != None and self.blip_transform != None:
            #     reference_img = self.preprocess(PIL.Image.open(reference_img_path))
            ...
            if self.split == 'val':
                ...
                target_img_path = str(self.img_paths[self.img_ids_indexes_map[target_img_id]])
...
class COCODataset(Dataset):
    ...
    def __init__(self, transform=None, blip_transform = None,root_dir='./coco/val2017/') -> None:
        super().__init__()
    ...
    def load_blip_sample(self, sample):
        ...
        fpath = str(os.path.join(self.root_dir, f'{val_img_id}.jpg'))
...
class VAWDataset(Dataset):
    ...
    def __init__(self, transform=None, image_dir='./Visual_Genome/VG_All/',blip_transform=None) -> None:
        super().__init__()
